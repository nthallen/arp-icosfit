...
sigosc = osc;
sigbeta = osc;
[osc(1,:), beta(1,:), sigosc(1,:), sigbeta(1,:)] = fit_comp([],S0, line_pos0, scans);
for i = 2:length(bases)
  [line_pos,S] = get_line_position(bases{i});
  [ osc(i,:), beta(i,:), sigosc(i,:), sigbeta(i,:) ] = fit_comp(S0, S, line_pos, scans);
  if nargout > 2
    S_out{i} = S;
    if nargout > 3
      sigosc_out = sigosc;
...
function [osc_out, beta_out, sigosc_out, sigbeta_out] = fit_comp(S0, S, line_pos, scans)
if isempty(scans)
  scans = S.scannum;
end
use_stat = 1;
