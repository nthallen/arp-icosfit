...
L = [10 50]; % Cavity length
R2_lim = [-1000 0];

R1 = [10 20 30 40 50 75 100 200 300 400 500 750 1000];
IM2 = ispcatalog;
IR2 = unique([IM2.R_cm]);
if ~isempty(R2_lim)
...
B = 0.1;
ICOS_margin = 0.1;
mnc = sprintf('MEl_B%d_rd%02d_th%.1f', round(B*10), floor(rd*100), th);
...
figure;
V = ddBL > 0.2 & Volume < 1 & L_tot < 60;
scatter(L(V),RLmin(V),[],Volume(V));
xlabel('Cell Length cm');
ylabel('RIM Length cm');
title('Methane/Ethane Configurations');
cb = colorbar;
cb.Label.String = 'Volume liters';
%
xt = get(gca,'xtick');
yt = get(gca,'ytick');
for i=1:length(xt)
...
figure;
V = ddBL > 0.2 & L_tot < 60; 
plot(L_tot(V),Volume(V),'*');
xlabel('RL+L cm');
ylabel('Volume l');
title('Possible Methane/Ethane Configurations');
%%
N_configs = 30;
ddBLok = ddBL > 0.2;
[~,VI] = sort(Volume);
